...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ОстаткиПродукции.Остатки КАК ОстаткиПродукцииОстатки
	|		ПО ПродукцияДокумента.Продукция = ОстаткиПродукцииОстатки.Номенклатура";
	...
	Если Режим = РежимПроведенияДокумента.Оперативный Тогда 
		...
		Запрос3.Текст = "ВЫБРАТЬ
		                ...
		                |					ИЗ
		                |						ПродукцияДокумента)
		                |				И ОбъектПредприятия = &ТорговаяТочка) КАК ОстаткиПродукцииОстатки"; 
	КонецЕсли;
